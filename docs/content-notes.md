...
- Superseded status:
  ...
  Superseded proposal (2026-08-27): one paragraph of roughly 100-150 words under its own heading at the bottom of About, below the bios and above the closing CTA.
  ...
  (4) Mitigate the pre-trained bias of AI against minority groups and women. The practice behind it, from the voice-product engagement (the one with the pilots), whose users were heavily minority and which assessed people on their conversations with the AI: the owner researched the ways the model could be racist, then designed a layer that normalised what a user said before it reached the grader, removing vernacular, gender cues and other signals that would tip the model off to a person's race, gender or group, so the assessment was independent of factors built into the training data. He also wrote a private research report for the client.
      Scope (2026-08-27): this is not an unconditional practice; POCs that assess nobody do not get the treatment. State the position at the level that is true: when a model's output is used to judge a person, its bias is our problem. Put the trigger in the heading (e.g. "When AI judges a person, its bias is our problem") and keep the example; stating the limit is credibility.
  ...
  Owner rewrite drafts (2026-08-27), with editorial notes: intro "AI should be used to make life better." adopted; "the people who meet it" anthropomorphised the model and "That is the whole test, and it rules some work out" was the aphorism reflex, an AI tell.
  The headings already say the list is refusals.
  Heading 2 alternative "AI is not medical advice (without expert validation)" (a hedge in a heading, and disclaimer register; the claim is a stance).
  Body 2 "We turned down an AI-voice talk therapy project because we couldn't be sure it was safe." adopted; note it drops the sharper reason, that he could not be the one deciding.
  Character line "We lead with curiosity, kindness, and determination" is not a red line and is three abstract nouns; if used, it belongs in the About intro in behavioural form (what those words look like from the client's side; "we don't stop until it works" carries grit).
  Heading settled (2026-08-27): "People are not the product" replaces "No race to the bottom"; it covers all three refusals (data, spam, attention) where the idiom covered one. The privacy line keeps the broad rule (invading people's privacy for corporate profit) with the owner's specific case as its example (selling users' data to third parties so ads can follow them around the internet); the rule is wider than the one case. Products bootstrapping their own datasets are fine; ad personalisation is a grey area and deliberately not legislated on the page.
  Refinement (2026-08-27): the close of this item is really about respecting people's attention, not stewardship of the internet. Attention is the reader's own, and it names what spam and privacy invasion have in common: treating a person as a resource to extract from. Close settled in the owner's words: "We build products that earn attention rather than take it." Distinct from the owner's planned first essay, which is about how working with AI agents fractures one's own attention and how to mitigate that; the site line is about respecting other people's attention. Do not link the two; the site's first writing link is a separate decision once the piece exists.
...
- NEVER add a short second sentence that delivers a verdict on the first ("That is the whole test, and it rules some work out.").
  The aphorism reflex.
  Say the thing once.
- Don't anthropomorphise AI ("the people who meet it").
  People do not meet software; they use it, talk to it, are assessed by it.
- Prefer the concrete event to the quality ("their own engineers took it from there", not "a team that could carry it"; "they kept us on", not "a lasting relationship").
  If a phrase sounds like a headline rather than something said on a call, rewrite it as what happened.
...
- Don't compress a fact into shorthand that needs the reader to know the domain ("a major exchange" for a crypto exchange; "preliminary commitment" for a first yes).
