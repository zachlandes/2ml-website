...
- Superseded status:
  ...
  (4) Mitigate the pre-trained bias of AI against minority groups and women. The practice behind it, from the voice-product engagement (the one with the pilots), whose users were heavily minority and which assessed people on their conversations with the AI: the owner researched the ways the model could be racist, then designed a layer that normalised what a user said before it reached the grader, removing vernacular and other cues that would tip the model off to a person's race or group, so the assessment was independent of factors built into the training data. He also wrote a private research report for the client.
